...
document.addEventListener('DOMContentLoaded', function() {
    ...
    // Set up form submission dengan event delegation
    document.addEventListener('submit', function(e) {
        if (e.target && e.target.id === 'sessionForm') {
            e.preventDefault();
            startSession();
        }
        
        if (e.target && e.target.id === 'batchForm') {
            e.preventDefault();
            startBatch();
        }
        
        if (e.target && e.target.id === 'addGoogleAccountForm') {
            e.preventDefault();
            addGoogleAccount();
        }
    });
...
function showGoogleAccountsModal() {
    const modal = document.getElementById('googleAccountsModal');
    if (modal) {
...
function switchGoogleTab(tab) {
    ...
    if (event && event.target) {
        ...
        // Find tab button and activate it
        const tabButtons = document.querySelectorAll('#googleAccountsModal .proxy-tab');
...
async function loadGoogleAccounts() {
    try {
        ...
        if (data.success) {
            console.log(`✅ Loaded ${data.accounts?.length || 0} Google accounts`);
            ...
            document.getElementById('googleAccountsBody').innerHTML = 
                '<tr><td colspan="5" style="color: #ef476f;">Error: ' + (data.error || 'Failed to load') + '</td></tr>';
        }
    } catch (error) {
        console.error('❌ Failed to load Google accounts:', error);
        document.getElementById('googleAccountsBody').innerHTML = 
            '<tr><td colspan="5" style="color: #ef476f;">Error connecting to server. Please check console.</td></tr>';
    }
}
...
function renderGoogleAccountsTable(accounts) {
    ...
    if (!accounts || accounts.length === 0) {
        html = '<tr><td colspan="5">No Google accounts added yet. Click "Add Account" to add your first account.</td></tr>';
    } else {
        accounts.forEach(account => {
            ...
            const successRate = account.successRate ? 
                `${Math.round(account.successRate * 100)}%` : 'N/A';
...
async function addGoogleAccount() {
    console.log('📝 Add Google Account function called');
    ...
    try {
        console.log(`📤 Sending request to add account: ${email}`);
        ...
        if (data.success) {
            ...
            document.getElementById('has2FA').checked = false;
            document.getElementById('backupCodesSection').style.display = 'none';
            
            // Switch back to accounts list
            switchGoogleTab('accounts');
            loadGoogleAccounts();
        } else {
