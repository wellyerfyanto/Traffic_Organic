...
class TrafficGenerator {
    ...
    async startOrganicSession(config) {
        ...
        this.activeSessions.set(sessionId, {
            ...
            keywords: config.customKeywords || [], // TAMBAH INI
            keywordMode: config.keywordMode || 'auto', // TAMBAH INI
            botDetectionCount: 0,
            timeMultiplier: this.getSessionTimeMultiplier(sessionId)
        });

        this.log(sessionId, 'ORGANIC_STARTED', 
            `Pure organic session started | Device: ${config.deviceType} | Proxy: ${config.proxyType} | Search: ${config.searchEngine} | Keyword Mode: ${keywordSource} | Time Multiplier: ${this.activeSessions.get(sessionId).timeMultiplier}x`
        );
    ...
    getAllSessions() {
        ...
        for (const [sessionId, session] of this.activeSessions) {
            sessions.push({
                ...
                keywordMode: session.keywordMode || 'auto', // TAMBAH INI
