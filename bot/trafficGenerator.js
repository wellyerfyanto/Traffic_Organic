// ==================== TRAFFIC GENERATOR 2025 (PROXY FIXED) ====================
...
class TrafficGenerator {
    constructor() {
        ...
        console.log('✅ Traffic Generator 2025 constructed (Proxy Fixed)');
    }
    ...
    getRandomUserAgent(deviceType, customAgent = null) {
        // Jika ada custom agent dari batch system, gunakan itu
        if (customAgent && customAgent.length > 20) {
            return customAgent;
        }
        
        // Desktop User Agents (Tested and Working)
        const desktopAgents = [
        ...
        // Mobile User Agents (Tested and Working)
        const mobileAgents = [
            ...
            if (config.proxyType && config.proxyType !== 'direct') {
                try {
                    ...
                    if (proxy && proxy.host && proxy.host !== 'direct') {
                        ...
                        console.log(`🔗 Using proxy: ${proxyUrl} (${proxy.displayName})`);
                    } else {
    ...
    async startOrganicSession(config) {
        ...
        this.log(sessionId, 'SESSION_STARTED', 
            `Session started | Engine: ${config.searchEngine} | Proxy: ${config.proxyType}`);
    ...
    async executeSession(sessionId, config) {
        ...
        try {
            ...
            // Step 3: Execute Search for Each Keyword
            ...
            for (let i = 0; i < keywords.length; i++) {
                ...
                try {
                    ...
                    // If requireCompletion is true and keyword failed, throw error
            ...
            // Check if all keywords completed successfully
    ...
    async performGoogleSearch(page, sessionId, keyword, multiplier = 2.5) {
        try {
            // Prepare Google URL with working parameters
            ...
            this.log(sessionId, 'GOOGLE_SEARCH', 
                `Searching: "${keyword}" (Delay: ${multiplier}x)`);
