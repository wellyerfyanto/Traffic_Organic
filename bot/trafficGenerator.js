// ==================== TRAFFIC GENERATOR 2025 (FULLY FIXED WITH KEYWORD MODE) ====================
...
class TrafficGenerator {
    constructor() {
        ...
        // Don't load modules immediately in constructor
        ...
        console.log('✅ TrafficGenerator constructed (lazy initialization)');
    }
    ...
    createFallbackHandlers() {
        console.log('⚠️ Creating fallback handlers for degraded mode');
        
        this.proxyHandler = {
            getProxyForSession: (sessionId, proxyType) => {
                console.log(`⚠️ Using fallback direct connection for ${sessionId}`);
                return {
        ...
        this.botHandler = {
            handleDetection: async () => {
                console.log('⚠️ Bot detection fallback handler');
                return 0;
            }
        };
        
        this.KeywordAnalyzer = class FallbackKeywordAnalyzer {
            async analyze() {
                return {
                    success: true,
                    keywords: ['organic', 'traffic', 'bot', 'seo', 'automation'],
                    stats: { totalFiltered: 5 }
                };
            }
        };
    }
    ...
    async startOrganicSession(config) {
        ...
        // For auto-loop sessions, use specific settings
        if (config.isAutoLoop) {
            this.log(sessionId, 'AUTO_LOOP_SESSION', 'This is an auto-loop initiated session');
            
            // Ensure sensible defaults for auto-loop
            if (!config.deviceType) config.deviceType = 'desktop';
            if (!config.maxKeywords) config.maxKeywords = 3;
            if (!config.enableSubUrl) config.enableSubUrl = true;
            
            // Randomize some parameters for diversity
            if (Math.random() > 0.5) {
        ...
        try {
            ...
            this.log(sessionId, 'PROXY_FALLBACK', 'Using direct connection as fallback');
        }
        ...
        this.log(sessionId, 'ORGANIC_STARTED', 
            `${sessionType} session started | Device: ${config.deviceType} | Proxy: ${config.proxyType} | Search: ${config.searchEngine} | Keyword Mode: ${keywordSource}`
        );
        
        // Execute organic flow in background
        this.executePureOrganicFlow(sessionId, config).then(() => {
            this.successfulSessions++;
            this.log(sessionId, 'ORGANIC_COMPLETED', `${sessionType} session completed successfully`);
        }).catch(error => {
            this.log(sessionId, 'ORGANIC_ERROR', `${sessionType} session failed: ${error.message}`);
    ...
    async executePureOrganicFlow(sessionId, config) {
        ...
        try {
            ...
            // Step 2: Keyword Analysis - ENHANCED WITH MANUAL MODE
            let keywords = [];
            
            // PRIORITY 1: Gunakan custom keywords jika mode manual
            if (config.keywordMode === 'manual' && 
                config.customKeywords && 
                config.customKeywords.length > 0) {
                
                keywords = config.customKeywords.slice(0, config.maxKeywords || 5);
                this.log(sessionId, 'KEYWORDS_MANUAL', 
                    `Using ${keywords.length} manual keywords: ${keywords.join(', ')}`);
            } 
            // PRIORITY 2: Auto analysis (default)
            else {
                ...
                if (!analysis.success || analysis.keywords.length === 0) {
                    this.log(sessionId, 'KEYWORD_ANALYSIS_FAILED', 
                        'No keywords found, using fallback keywords');
                    // Fallback keywords
                    keywords = ['crypto', 'bitcoin', 'blockchain', 'trading', 'news']
                        .slice(0, config.maxKeywords || 5);
                } else {
                    keywords = analysis.keywords.slice(0, config.maxKeywords || 5);
                }
            ...
            session.keywordMode = config.keywordMode;
            ...
            for (let i = 0; i < keywords.length; i++) {
                ...
                this.log(sessionId, 'KEYWORD_START', 
                    `[${i+1}/${keywords.length}] Searching: "${keyword}" via ${config.searchEngine} (Mode: ${config.keywordMode})`);
            ...
            this.log(sessionId, 'ORGANIC_COMPLETE', 
                `Completed ${keywords.length} organic searches via ${config.searchEngine} (Keyword Mode: ${config.keywordMode})`);
    ...
    async launchBrowser(sessionId, config) {
        try {
            ...
            const proxyArgs = proxy.puppeteerArgs || [
                '--no-sandbox',
                '--disable-setuid-sandbox',
                '--disable-dev-shm-usage',
    ...
    async setupAndTestProxiesForSession(sessionId, config) {
        if (!this.proxyHandler) {
            throw new Error('Proxy handler not available. Call initialize() first.');
        }
    ...
    getAllSessions() {
        ...
        for (const [sessionId, session] of this.activeSessions) {
            sessions.push({
                ...
                keywordMode: session.keywordMode || 'auto',
