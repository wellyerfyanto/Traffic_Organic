...
class KeywordAnalyzer {
    ...
    // MODULE 2: Contextual Suggestions - UPDATED WITH REAL API
    async getContextualSuggestions(seedKeywords) {
        ...
        for (const keyword of seedKeywords.slice(0, 3)) {
            try {
                ...
                console.log(`⚠️ Failed to get suggestions for: ${keyword}, using mock data`);
    ...
    async fetchGoogleAutocomplete(keyword) {
        try {
            // Real Google Autocomplete API
            const url = `https://suggestqueries.google.com/complete/search?client=firefox&q=${encodeURIComponent(keyword)}&hl=id`;
            ...
            // Google API returns JSONP format, parse it
            const match = text.match(/window\.google\.ac\.h\((.*)\)/);
            if (match && match[1]) {
                const data = JSON.parse(match[1]);
                if (data[1] && Array.isArray(data[1])) {
                    return data[1].slice(0, 5).map(item => item[0]);
                }
            }
            ...
            console.log('⚠️ Google Autocomplete failed, using mock data');
            // Fallback ke mock data
    ...
    async fetchRelatedSearches(keyword) {
        try {
            // Untuk sekarang, kita gunakan mock data dulu
            // Bisa ditingkatkan dengan scraping Google "Searches related to"
            const mockRelated = [
            ...
            console.log('⚠️ Related searches failed, using mock data');
            return [];
        }
    }
