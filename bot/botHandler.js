...
class BotHandler {
  constructor() {
    ...
    this.strategies = [
      ...
      this.simulateSearchInPage, // ✅ STRATEGI BARU
      this.simulateLinkScanning   // ✅ STRATEGI BARU
    ];
  }
  ...
  // ==================== NEW STRATEGY: SIMULATE SEARCH IN PAGE ====================
  async simulateSearchInPage(page, context) {
    if (!context.includes('google')) return;
    
    try {
      console.log('🔍 Simulating "Search in Page" behavior...');
      
      // Random chance untuk simulate Ctrl+F
      if (Math.random() < 0.3) {
        // Simulate press Ctrl+F
        ...
        const searchTerms = ['crypto', 'news', 'blog', 'article', 'update', 'latest'];
        const randomTerm = searchTerms[Math.floor(Math.random() * searchTerms.length)];
        
        await page.keyboard.type(randomTerm, { delay: 50 + Math.random() * 100 });
        ...
        console.log(`✅ Simulated search in page for: ${randomTerm}`);
      }
      
      // Simulate visual scanning
      await page.evaluate(() => {
        // Scroll dengan pattern natural
        window.scrollBy({ 
      ...
      // Simulate mouse hover over links
      await page.evaluate(() => {
        ...
        if (links.length > 0) {
          ...
          randomLink.dispatchEvent(event);
        }
      });
      
    } catch (error) {
      console.log('⚠️ Search in page simulation error:', error.message);
    }
  }

  // ==================== NEW STRATEGY: SIMULATE LINK SCANNING ====================
  async simulateLinkScanning(page, context) {
    try {
      console.log('👁️ Simulating link scanning behavior...');
      
      // Simulate reading links
      await page.evaluate(() => {
        // Create visual scanning pattern
        const links = document.querySelectorAll('a');
        links.forEach((link, index) => {
          if (index < 5 && Math.random() < 0.5) {
            // Add slight visual feedback
            link.style.transition = 'background-color 0.3s';
            link.style.backgroundColor = 'rgba(0, 120, 215, 0.05)';
      ...
      // Simulate reading time per link
      await page.waitForTimeout(800 + Math.random() * 1200);
      
      // Simulate decision making delay
      await page.waitForTimeout(1000 + Math.random() * 2000);
      
      console.log('✅ Simulated link scanning');
      
    } catch (error) {
      console.log('⚠️ Link scanning simulation error:', error.message);
    }
  }

  // ==================== EXISTING STRATEGIES (TETAP SAMA) ====================
