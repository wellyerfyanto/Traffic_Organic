...
try {
  ...
  const loadModule = (modulePath) => {
    try {
      ...
      if (isRailway) {
        const altPath = modulePath.replace('./bot/', './');
        try {
  ...
  ProxyHandler = loadModule('./bot/proxyHandler');
  KeywordAnalyzer = loadModule('./bot/keywordAnalyzer');
  BotHandler = loadModule('./bot/botHandler');
  
  // Load TrafficGenerator
  const TrafficGenerator = loadModule('./bot/trafficGenerator');
...
const AUTO_LOOP_CONFIG = {
  ...
  proxyRefreshInterval: 5 * 60 * 1000, // 5 minutes instead of 10
  minProxyCount: 10,
