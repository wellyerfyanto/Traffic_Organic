...
try {
  ...
  // ==================== FIXED: PROPER MODULE LOADING ====================
  // Load modules with proper error handling
  const modules = [
    { name: 'proxyHandler.js', variable: 'ProxyHandler' },
    { name: 'keywordAnalyzer.js', variable: 'KeywordAnalyzer' },
    { name: 'botHandler.js', variable: 'BotHandler' },
    { name: 'trafficGenerator.js', variable: 'TrafficGenerator' }
  ];

  const loadedModules = {};

  for (const module of modules) {
    const modulePath = path.join(botDir, module.name);
    console.log(`\nLoading ${module.name}...`);
    console.log(`Path: ${modulePath}`);
    console.log(`File exists: ${fs.existsSync(modulePath)}`);

    if (!fs.existsSync(modulePath)) {
      console.error(`❌ ${module.name} not found at ${modulePath}`);
      
      // Try to find in root directory as fallback
      const rootPath = path.join(__dirname, module.name);
      if (fs.existsSync(rootPath)) {
        console.log(`Trying root directory: ${rootPath}`);
        try {
          loadedModules[module.variable] = require(rootPath);
          console.log(`✅ ${module.name} loaded from root directory`);
        } catch (error) {
          console.error(`❌ Failed to load ${module.name} from root: ${error.message}`);
        }
      } else {
        console.error(`❌ ${module.name} not found anywhere`);
      }
      continue;
    }

    try {
      loadedModules[module.variable] = require(modulePath);
  ...
  ProxyHandler = loadedModules.ProxyHandler;
  KeywordAnalyzer = loadedModules.KeywordAnalyzer;
  BotHandler = loadedModules.BotHandler;
  const TrafficGenerator = loadedModules.TrafficGenerator;

  // Check if all required modules are loaded
  if (!ProxyHandler || !KeywordAnalyzer || !BotHandler || !TrafficGenerator) {
    throw new Error('One or more bot modules failed to load');
  }
  ...
  try {
    ...
    if (botManager.proxyHandler && botManager.proxyHandler.initialize) {
      botManager.proxyHandler.initialize().then(() => {
        console.log('✅ Proxy system ready with VPN support');
      }).catch(err => {
