...
class BatchSessionManager {
  constructor() {
    ...
    this.failedSessions = [];
    this.maxConcurrent = 5; // Maksimal 5 session bersamaan
    this.maxTotal = 1000;   // Maksimal 1000 session total
  ...
  startBatch(batchId) {
    ...
    // Start initial sessions up to concurrent limit
    this.startNextSessions(batchId);
    
    // Setup interval to check and start new sessions
  ...
  startNextSessions(batchId) {
    ...
    // Count currently running sessions in this batch
    const runningCount = batch.sessions.filter(s => s.status === 'running').length;
    
    // Calculate how many more can be started
    ...
    for (const batch of this.batchQueue) {
      if (batch.status === 'running') {
        ...
        if (remainingSessions.length === 0) {
          ...
          // Stop interval if all batches are done
...
(async () => {
  try {
    ...
    try {
      ...
      global.botManager = botManager; // Make available globally
...
app.post('/api/start-organic', async (req, res) => {
  try {
    const { targetUrl, proxyType, deviceType, searchEngine, keywordMode, customKeywords } = req.body;
...
const server = app.listen(PORT, () => {
  console.log('\n' + '='.repeat(70));
  console.log(`🚀 Organic Traffic Bot 2025 - Batch Session Edition`);
  ...
  console.log('   POST /api/batch/start/:id  - Start batch execution');
  console.log('   GET  /api/batch/status/:id - Check batch status');
  console.log('   GET  /api/all-sessions     - List all sessions');
  console.log('   POST /api/proxies/refresh  - Refresh proxy pool');
  console.log('\n✅ Server is ready! Batch System: 5 concurrent, 1000 max sessions');
});
