...
// ==================== PUPPETEER FALLBACK CONFIGURATION ====================
let puppeteer = null;
try {
  ...
  console.log('📦 Attempting to use puppeteer-core...');
  ...
  try {
    ...
    console.error('❌ Could not load puppeteer-core:', coreError.message);
    console.log('⚠️  Running in API-only mode without browser automation');
  }
}
...
try {
  // Load bot modules with retry and multiple paths
  const loadModule = (moduleName) => {
    ...
    for (const modulePath of possiblePaths) {
      try {
        ...
        console.log(`✅ Loaded ${moduleName} from: ${modulePath}`);
        return module;
      } catch (error) {
  ...
  // Try to load modules
  ProxyHandler = loadModule('proxyHandler');
  KeywordAnalyzer = loadModule('keywordAnalyzer');
  BotHandler = loadModule('botHandler');
  
  // Load TrafficGenerator
  const TrafficGenerator = loadModule('trafficGenerator');
...
if (botManager) {
  ...
  }, 2000); // Delay to ensure everything loaded
} else {
